...
def update_fantasy_settings(req: FantasySettingsRequest):
    try:
        ...
        # 取得前端傳來的新聯盟與新球隊名稱
        nl = req.league_name.strip()
        nt = req.team_name.strip()
        
        if "leagues" not in db:
            db["leagues"] = {}
            
        # 1. 確保聯盟存在 (如果輸入新聯盟名字，就新建一個房間)
        if nl not in db["leagues"]:
            db["leagues"][nl] = {"teams": {}, "weights": req.weights, "ignored_players": []}
            
        # 2. 🚀 確保球隊存在 (輸入新球隊就新建空名單；絕對不用 pop 刪除舊球隊！)
        if nt not in db["leagues"][nl]["teams"]:
            db["leagues"][nl]["teams"][nt] = []
            
        # 3. 更新該聯盟的計分權重
        db["leagues"][nl]["weights"] = req.weights
        ...
        # 無視 Firebase 權限警告，只要本機成功就給綠燈
        if local_ok: 
            return {"status": "success", "message": f"✅ 設定儲存成功！目前位於: {nl} - {nt}"}
        else: 
